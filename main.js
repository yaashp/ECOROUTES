/* ═══════════════════════════════════════════════════
   ...
   Sections:
     ...
     3. Page Navigation
     4. Eco Features (Particles, Ripple, Timeline)
═══════════════════════════════════════════════════ */
...
function updateEquivalents(totCO2) {
  if (totCO2 <= 0) {
    ['eqCar','eqTrees','eqBulb'].forEach(id => {
      document.getElementById(id).textContent = '—';
    });
    return;
  }
  // 1 km petrol car ≈ 0.192 kg CO₂ (DEFRA)
  document.getElementById('eqCar').textContent = (totCO2 / 0.192).toFixed(1) + ' km';

  // Precise tree photosynthesis calculation:
  // A mature tree absorbs ~21.77 kg CO₂/year (US Forest Service / peer-reviewed average)
  // That is 21.77 / 12 = 1.814 kg CO₂ absorbed per tree per month
  // Trees needed (exact) = totCO2 / 1.814, displayed with 2 decimal places
  // e.g. 0.5 kg CO₂ → 0.28 trees (not rounded up to 1, giving false precision)
  const kgPerTreePerMonth = 21.77 / 12; // 1.814 kg/tree/month
  const treesNeeded = totCO2 / kgPerTreePerMonth;
  // Show as integer if ≥ 10, 1 decimal if 1–9, 2 decimals if < 1
  let treesDisplay;
  if (treesNeeded >= 10) {
    treesDisplay = Math.round(treesNeeded).toLocaleString();
  } else if (treesNeeded >= 1) {
    treesDisplay = treesNeeded.toFixed(1);
  } else {
    treesDisplay = treesNeeded.toFixed(2);
  }
  document.getElementById('eqTrees').textContent = treesDisplay;

  // Electric kettle boiling 1 cup of tea (250 ml, ~0.04 kWh)
  // India grid: ~0.71 kg CO₂/kWh → 0.04 × 0.71 = 0.0284 kg CO₂ per cup ≈ 0.028
  document.getElementById('eqBulb').textContent = Math.round(totCO2 / 0.028).toLocaleString() + ' cups';
}
...
/* ═════════════════════════════════════════════════════
   MAP VIEW TOGGLE
═════════════════════════════════════════════════════ */
let currentMapView = 'leaflet'; // default to tile map
...
═════════════════════════════════════════════════════ */
initMap();
// Tile map is default — invalidate size after DOM is fully painted
...
// Override addSelectedFoodsToCalculator to also switch to calculator page
window.addEventListener('load', function() {
  var origAdd = window.addSelectedFoodsToCalculator;
  if (origAdd) {
    window.addSelectedFoodsToCalculator = function() {
      origAdd();
...
/* ═══════════════════════════════════════════════════════════
  ...
  10. Page-switch transition hook
═══════════════════════════════════════════════════════════ */
